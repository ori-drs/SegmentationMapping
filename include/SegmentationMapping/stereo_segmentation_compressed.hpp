...
namespace SegmentationMapping {
  ...
  class StereoSegmentationCompressed {
  public:
    StereoSegmentationCompressed()
      : nh_()
      ...
      , it_(nh_)
    {
      ...
      color_sub_ = new ImageSubscriber(it_, color_topic_, 50);
      depth_sub_ = new ImageSubscriber(it_, depth_topic_, 50);
      label_sub_ = new ImageSubscriber(it_, label_topic_, 50);
#else
      color_sub_ = new ImageSubscriber(pnh, color_topic_, 50);
      depth_sub_ = new ImageSubscriber(pnh, depth_topic_, 50);
      label_sub_ = new ImageSubscriber(pnh, label_topic_, 50);
#endif
      ...
      sync_->registerCallback(boost::bind(&StereoSegmentationCompressed::DepthColorCallback, this,_1, _2, _3, _4, _5));
  ...
  template <unsigned int NUM_CLASS>
  inline void
  StereoSegmentationCompressed<NUM_CLASS>::DepthColorCallback(const sensor_msgs::ImageConstPtr& depth_msg,
                                                              const sensor_msgs::ImageConstPtr& color_msg,
                                                              const sensor_msgs::CameraInfoConstPtr& camera_info_msg,
                                                              const sensor_msgs::ImageConstPtr& labeled_msg,
                                                              const ImageLabelDistributionConstPtr & distribution_msg) {
    ...
    // ROS_WARN("fx: %f, fy: %f", fx, fy);

    // ROS_WARN("label image: cols:%i, rows: %i", label_ptr->image.cols, label_ptr->image.rows);
    // ROS_WARN("color image: cols:%i, rows: %i", color_ptr->image.cols, color_ptr->image.rows);
    // ROS_WARN("depth image: cols:%i, rows: %i", depth_ptr->image.cols, depth_ptr->image.rows);
    ...
    // ROS_WARN("loaded distribution data");
    ...
    // ROS_WARN("rows: %i, cols %i, num_classes: %i", rows, cols, NUM_CLASS);
  ...
  template <unsigned int NUM_CLASS>
  inline void
  StereoSegmentationCompressed<NUM_CLASS>::Depth2PointCloud1(const sensor_msgs::ImageConstPtr& depth_msg,
                                                             const sensor_msgs::ImageConstPtr& color_msg,
                                                             bool publish_semantic,
                                                             const cv::Mat & label_img,
                                                             const cv::Mat & distribution_img) {
    std::cout<<"New image with label\n";
    // ROS_WARN("New image with label");
  ...
  template <unsigned int NUM_CLASS>
  inline void
  StereoSegmentationCompressed<NUM_CLASS>::Depth2PointCloud2(const sensor_msgs::ImageConstPtr& depth_msg,
                                                             const sensor_msgs::ImageConstPtr& color_msg,
                                                             bool publish_semantic,
                                                             const cv::Mat & label_img) {
   // Set up to-publish cloud msg
